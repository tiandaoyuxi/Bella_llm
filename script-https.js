document.addEventListener('DOMContentLoaded', function () {
    ...
    import('./api-config.js').then(module => {
        ...
        // 如果无法加载配置，使用默认HTTP
        initApp({
            currentUrl: 'http://localhost:3002/api',
            getUrl: (endpoint) => `http://localhost:3002/api/${endpoint}`
        });
    });

    // 主应用初始化函数
    function initApp(apiConfig) {
        // --- 加载屏幕处理 ---
        console.log('🚀 页面开始加载...');
        ...
        async function checkTTSStatus() {
            try {
                const response = await fetch(apiConfig.getUrl('tts/status'));
                ...
                // 尝试使用HTTP
                if (apiConfig.currentUrl === apiConfig.baseUrl) {
                    console.log('尝试使用HTTP连接TTS服务...');
                    apiConfig.useHttp();
                    ...
                    try {
                        ...
                        const data = await response.json();
                        ttsServiceAvailable = data.status === 'connected';
                        console.log(ttsServiceAvailable ? '🔊 TTS服务已通过HTTP连接' : '🔇 TTS服务未连接');
                    } catch (fallbackError) {
                        console.log('🔇 TTS服务HTTP连接也失败:', fallbackError.message);
        ...
        async function playTTS(text) {
            ...
            try {
                ...
                const response = await fetch(apiConfig.getUrl('tts'), {
                    method: 'POST',
                    headers: {
                ...
                if (apiConfig.currentUrl === apiConfig.baseUrl) {
                    ...
                    try {
                        ...
                        console.error('HTTP TTS播放也失败:', fallbackError);
                    }
                }
        ...
        async function loadAvailableModels() {
            try {
                const response = await fetch(apiConfig.getUrl('models'));
                const data = await response.json();
                ...
                if (apiConfig.currentUrl === apiConfig.baseUrl) {
                    ...
                    try {
                        ...
                        console.error('HTTP加载模型列表也失败:', fallbackError);
                    }
                }
        ...
        async function checkOllamaStatus() {
            try {
                const response = await fetch(apiConfig.getUrl('status'));
                const data = await response.json();
                ...
                console.log('💭 Ollama服务未连接，尝试HTTP连接...');
                
                // 如果HTTPS失败，尝试HTTP
                if (apiConfig.currentUrl === apiConfig.baseUrl) {
                    console.log('尝试通过HTTP连接Ollama服务...');
                    apiConfig.useHttp();
                    ...
                    try {
                        ...
                        console.error('HTTP连接Ollama服务也失败:', fallbackError);
                        isLLMEnabled = false;
                    }
                } else {
                    isLLMEnabled = false;
        ...
        async function chatWithBella(message) {
            ...
            try {
                ...
                const response = await fetch(apiConfig.getUrl('chat'), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                // 如果HTTPS失败，尝试HTTP
                if (apiConfig.currentUrl === apiConfig.baseUrl) {
                    console.log('尝试通过HTTP与贝拉对话...');
                    apiConfig.useHttp();
                    
                    try {
                        isThinking = false;
                        return await chatWithBella(message);
                    } catch (fallbackError) {
                        console.error('HTTP与贝拉对话也失败:', fallbackError);
        ...
        function analyzeAndReact(text) {
            ...
            if (positiveWords.some(word => lowerText.includes(word))) {
                ...
                playSpecificVideo(positiveVideos[randomIndex]);
                return 'positive';
            }
            ...
            // 检查是否包含思考性词汇
            if (lowerText.includes('为什么') || lowerText.includes('如何') || lowerText.includes('什么')) {
                playSpecificVideo(thoughtfulVideo);
                return 'thoughtful';
            }
        ...
        function loadTTSSettingsToUI() {
            ...
            const defaultVoiceGroup = document.getElementById('default-voice-group');
            ...
            // 显示/隐藏相应的音色选择组
            if (ttsSettings.voiceType === 'custom') {
                defaultVoiceGroup.classList.add('hidden');
                customVoiceGroup.classList.remove('hidden');
            } else {
                defaultVoiceGroup.classList.remove('hidden');
        ...
        function saveTTSSettings() {
            ...
            ttsSettings.voiceType = voiceTypeSelect.value;
            ttsSettings.defaultVoice = defaultVoiceSelect.value;
            ttsSettings.customVoice = customVoiceSelect.value;
            ttsSettings.speed = parseFloat(speedSlider.value);
        ...
        if (cancelTTSSettingsBtn) {
            cancelTTSSettingsBtn.addEventListener('click', () => {
                hideTTSSettingsModal();
            });
        }
        ...
        if (testTTSBtn) {
            testTTSBtn.addEventListener('click', () => {
                // 获取当前设置的音色
                const voiceType = document.getElementById('voice-type-select').value;
                const defaultVoice = document.getElementById('default-voice-select').value;
                const customVoice = document.getElementById('custom-voice-select').value;
                const speed = parseFloat(document.getElementById('speed-slider').value);

                const selectedVoice = voiceType === 'custom' ? customVoice : defaultVoice;
                const testText = `这是${selectedVoice}音色的测试，语速为${speed}。`;

                // 临时使用当前设置播放测试语音
                const originalSettings = { ...ttsSettings };
                ttsSettings.voiceType = voiceType;
                ttsSettings.defaultVoice = defaultVoice;
                ttsSettings.customVoice = customVoice;
                ttsSettings.speed = speed;

                playTTS(testText).then(() => {
                    // 恢复原始设置
                    ttsSettings = originalSettings;
                });
            });
        }
        ...
        function initializeApp() {
            // 设置好感度初始值
            favorabilityBar.style.width = '65%';
